...
namespace WebApplication3.App_Start
{
    ...
    public class AutoFacConfig
    {
        public static void Register() {
            //第一步；构造一个autofac的 builder容器
            ContainerBuilder builder = new ContainerBuilder();
            //第二步：告诉AutoFac控制器工厂，控制器类的创建去哪些程序集中查找（默认控制器工厂是去扫描bin目录下的所有程序集）
            //2.1 从当前运行的bin目录下加载UserManagement.dll程序集
            Assembly ControllerAss = Assembly.Load("UserManagement");
            ...
            //3.2 反射扫描这个UserManagement.dll程序集中所有的类，得到这个程序集中所有类的集合。
            Type[] rtypes = ControllerAss.GetTypes();
